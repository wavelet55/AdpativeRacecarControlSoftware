...
//Only return true when Nema Sentence contains Lat-Lon-Alt or velocity
//updates.... otherwise return false.  The other received info will be
//posted in the GPS-Fix structure and will show-up with the lat-lon ... data.
bool GPSService::parseNmeaSentence(const NMEASentence& nmea)
{
    bool validNemaCmd = false;
    if(nmea.name == "PSRF150")
    {
        read_PSRF150(nmea);
        validNemaCmd = false;
    }
    else if(nmea.name == "GPGGA")
    {
        read_GPGGA(nmea);
        validNemaCmd = false;
    }
    else if(nmea.name == "GPGSA")
    {
        read_GPGSA(nmea);
        validNemaCmd = false;
    }
    else if(nmea.name == "GPGSV")
    {
        read_GPGSV(nmea);
        validNemaCmd = false;
    }
    else if(nmea.name == "GPRMC")
    {
        read_GPRMC(nmea);
        validNemaCmd = true;        //Publish GPS when Lat/Lon/alt & velocity are up-to-date
    }
    else if(nmea.name == "GPVTG")
    {
        read_GPVTG(nmea);
        validNemaCmd = false;
    }
