...
void NMEAParser::squish(string& str){
...
bool NMEAParser::parseGpsNmeaString(std::string &gpsStr, NMEASentence &nmea)
{
    bool error = true;
    ios_base::fmtflags oldflags = cout.flags();

    //Remove any whitespace
    NMEAParser::squish(gpsStr);

    try
    {
        parseText(nmea, gpsStr);
    }
    catch (NMEAParseError&)
    {
        error = true;
...
void NMEAParser::parseText(NMEASentence& nmea, string &txt){
	...
	if (haschecksum){
		...
		std::string csStr = txt.substr(0, checkstri);
		nmea.calculatedChecksum = calculateChecksum(csStr);
	}
	else
	{
