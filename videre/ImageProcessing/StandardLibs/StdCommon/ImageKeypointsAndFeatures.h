...
namespace ImageProcLibsNS
{
    ...
    class ImageKeypointsAndFeatures
    {
    private:
        //Logging System
        log4cxx::LoggerPtr log4cpp_;

        cv::Ptr<cv::FeatureDetector> _featureDetectorPtr;
    ...
    public:
        ...
        int getNumerOfKeyPoints()  {return KeyPointsVec.size();}

        //An openCV Matrix of Features
        cv::Mat FeaturesMat;

        //Set to true if the features have been computed and are available.
        ...
        ImageKeypointsAndFeatures(FeatureDetectorType_e fdt = FeatureDetectorType_e::FDT_ORB);

        ~ImageKeypointsAndFeatures();
        ...
        bool generateKeypointsOnly(cv::InputArray img,
        ...
        //returns false if generated ok, true if there was an error
        //generating the keypoints and images.
        bool generateKeypointsAndFeatures(cv::InputArray img,
                                          cv::InputArray mask = cv::noArray());
        ...
        void releaseFeatureDetector()
        {
            ...
            {
                _featureDetectorPtr.release();
            }
        }

        //Modify the image by adding the keypoint locations to the image.
        bool markImageWithKeypoints(cv::InputOutputArray img, bool drawRichKeypts = false);
