...
**************************************************************************/
    ...
    */
    ...
    CBlobResult::CBlobResult(cv::Mat *source, cv::Mat *mask, int threshold, bool findmoments)
    {
        bool success;

        try
        {
            //IplImage iplImgSource = IplImage(*source);
            if (mask != nullptr)
            {
                //IplImage iplImgMask = IplImage(*mask);
            ...
            else
            {
                ...
                success = BlobAnalysis(source, (uchar) threshold, nullptr, true, findmoments, m_blobs);
            }
        }
        catch (...)
        {
