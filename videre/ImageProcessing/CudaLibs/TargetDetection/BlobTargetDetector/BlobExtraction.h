...
#if !defined(_CUDA_CLASSE_BLOBEXTRACTION_INCLUDED)
#define _CUDA_CLASSE_BLOBEXTRACTION_INCLUDED

namespace CudaBlobTargetDetectorNS
{
    //! Extreu els blobs d'una imatge
    bool BlobAnalysis(cv::Mat *inputImage, uchar threshold, cv::Mat *maskImage,
                      bool borderColor, bool findmoments, blob_vector &RegionData);

    // FUNCIONS AUXILIARS

    //! Fusiona dos blobs
    void Subsume(blob_vector &RegionData, int, int *, CBlob_cuda *, CBlob_cuda *, bool, int, int);
    ...
    double GetExternPerimeter(int start, int end, int row, int width, int height, cv::Mat *maskImage);
